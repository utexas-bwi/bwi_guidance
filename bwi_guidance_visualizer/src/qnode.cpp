...
#include <sstream>
#include <bwi_guidance_visualizer/qnode.hpp>
...
namespace bwi_guidance_visualizer {
  ...
  bool QNode::init() {
    ros::init(init_argc,init_argv,"bwi_guidance_visualizer");
    ros::start(); // explicitly needed since nodehandle going out of scope

    // Get the robot positioner service
    ros::NodeHandle nh;
    // robot_positioner_ = nh.serviceClient<bwi_guidance_msgs::PositionRobot>("position");
    ...
    bwi_guidance::readExperimentFromFile(experiment_file_, experiments_);
    bwi_guidance::getInstanceNames(experiments_, experiment_box_strings_);
    
    /* Process user data */
    bwi_guidance::readUserDataFromFile(users_file_, users_);
    ...
    bwi_guidance::computeOrderings(experiments_, orderings);
    ...
    for (size_t exp = 0; exp < experiment_box_strings_.size(); ++exp) {
      std::vector< std::vector<bwi_guidance::LocationStamped> >
        all_paths_for_experiment;
      for (size_t u = 0; u < users_.size(); ++u) {
        std::vector<bwi_guidance::LocationStamped> path;
        std::string file = data_directory_ + "/" + users_[u].id + "_" + 
          experiment_box_strings_[exp];
        bwi_guidance::readOdometry(file, path);
  ...
  void QNode::updateFrame(int current_time_step) {
    ...
    bwi_guidance::Instance& exp = 
      bwi_guidance::getInstance(experiments_, current_experiment_index_);
